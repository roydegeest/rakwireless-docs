module.exports = [
    { text: 'Home', link: '/en-us/' },
    { text: 'Quick Start', link: '/en-us/quick-start/' },
    //{ text: 'Datasheet', link: '/en-us/datasheet/' },
    // { text: 'RUI', link: '/en-us/rui/' },
    // { text: 'Downloads', link: 'https://downloads.rakwireless.com/' },
    // { text: 'Store', link: 'https://store.rakwireless.com/' },
    {text: 'User Manual', link: '/en-us/user-manual/'},
    {text: 'Deployment Guide', link : '/en-us/deployment-guide/'},
    { text: 'Datasheet', link: '/en-us/datasheet/rak831/#rak831-lpwan-gateway-concentrator' },
    { text: 'Forum', link: 'https://forum.rakwireless.com/' }
]